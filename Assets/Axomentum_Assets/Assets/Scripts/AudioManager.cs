...
public class AudioManager : MonoBehaviour
{
    ...
    void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        switch (scene.name)
        {
            ...
            default:
                ...
                Debug.LogWarning($"Sahne '{scene.name}' i�in m�zik atanmam��.");
                return;
        }
    ...
    public void PlayWorldMusic(int worldIndex)
    {
        if (worldIndex >= 0 && worldIndex < worldMusicTracks.Length && worldIndex != currentWorldIndex)
        {
            ...
            {
                Debug.LogWarning($"World {worldIndex} i�in m�zik klibi veya AudioSource bulunamad�!");
            }
        }
        else if (worldIndex < 0 || worldIndex >= worldMusicTracks.Length)
        {
            Debug.LogError($"Ge�ersiz d�nya index'i: {worldIndex}");
        }
    }
}